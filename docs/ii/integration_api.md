...
Integration API используется для интеграции AppSec.Hub с другими приложениями.

## Запуск импорта результатов сканирования

Реализует загрузку результатов сканирования из инструмента (например, Checkmarx или Nexus IQ) в AppSec.Hub. При этом, на стороне AppSec.Hub создаются все необходимые объекты и связи (приложение, кодовая база, артефакт, пайплайн и т. д.), запускается импорт уязвимостей, осуществляется проверка прохождения QG. Однако само сканирование НЕ ВЫПОЛНЯЕТСЯ.

!!! note "Примечание"
    Данный сервис только запускает процесс импорта, который выполняется в асинхронном режиме. Проверка статуса запуска проверяется с помощью сервиса Get scan status.
...
`unit`|String|Связывает артифакт/кодовую базу со структурным подразделением. Если указанное структурное подразделение отсутствует, оно создается
