...
Параметр|Обязат. параметр|Описание|Пример
-|-|-|-
...
Параметр	Обязат. параметр	Описание	Пример
--artifact-url	–	URL артефакта, хранящегося в Nexus RM или файловом хранилище. Если URL указан, другие параметры типов артефактов, игнорируются	--artifact-url https://nexus.dev.swordfishsecurity.com/
repository/web-maven-test/com/mkyong/web/java-web-project/1.15/
java-web-project-1.15.war
...
Параметр	Обязат. параметр	Описание	Пример
--repository-url	+	URL репозитория артефакта	--repository-url https://nexus.dev.swordfishsecurity.com/
repository/web-raw-test
...
--cx-team	–	Имя команды Checkmarx	--cx-team CxServer\DEV\DVJA
...
--checkout-path 	+	Относительный путь до кодовой базы	--/ 
or 
--/ /plugin/
--commit	–	 	 
--branch-filter	–	Фильтр сканируемых ветвей. Может применяться к основной и дополнительной кодовым базам. ‎Значение по умолчанию: master 	--branch-filter feature/*
--branch-filter *
...
--codebase-type	–	Тип VCS type. Возможное значение: git ‎Значение по умолчанию: git	--codebase-type git
--codebase-build-tool	–	Инструмент сборки исходного кода. Возможные значение: N/A, maven, gradle, nuget, npm, pip‎. Значение по умолчанию: maven	--codebase-build-tool nuget
