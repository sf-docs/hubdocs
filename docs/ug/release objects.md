...
Некоторые инструменты поддерживают механизм тегирования. Тегирование позволяет наносить метки на релизные объекты в зависимости от результатов сканирования. К примеру, если в результате анализа библиотек с открытым исходным кодом (с применением инструментов класса SCA) релизный объект успешно проходит Quality, он будет помечен меткой «QG_SCA_PASS», а в противном случае — меткой «QG_SCA_FAIL». Отсутствие метки на релизном объекте при определенном QG означает, что релизный объект еще не сканировался после установки QG. Если QG не задан, а сканирование артефактов было выполнено, артефакт не будет помечен тегом в результате этого сканирования.
