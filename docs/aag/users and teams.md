...
Также существует возможность отфильтровать существующих пользователей, используя раскрывающиеся меню **with auth type** и **with state**. Выберите пункты Local users, LDAP users или All auth types в меню **with auth type**. На экране появится список пользователей с соответствующим типом аутентификации. Выберите пункты **Active users**, **Non active users** или **All states** в меню **with state**. На экране появится список пользователей с соответствующим состоянием.
...
Поле **Base DN for roles** определяет Base DN в терминах LDAP. Введите фильтр в поле **Filter for groups and roles** и нажмите кнопку **Create** в нижней части вкладки. Сообщение, подтверждающее обновление сопоставления ролевой модели, появится в правом нижнем углу экрана.
...
Сопоставление групп определяет соответствие между группами LDAP и командами в AppSec.Hub. Если в системе произведено сопоставление групп, команды в AppSec.Hub назначаются LDAP пользователям автоматически.
